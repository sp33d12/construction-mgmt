...
const DEFAULT = { order_number: '', order_date: '', subject: '', issued_to: '', project_id: '', file_data: '', file_name: '', file_type: '' };
...
function FilePreview({ file_data, file_name, file_type }) {
  ...
  return (
    <div className="h-40 bg-purple-50 rounded-xl flex flex-col items-center justify-center gap-2 border border-purple-100">
      <span className="text-5xl">📄</span>
      <span className="text-xs text-purple-600 font-semibold px-2 text-center truncate w-full text-center">{file_name}</span>
    </div>
  );
}
...
export default function AdminOrders() {
  ...
  const openEdit = item => {
    setEditItem(item);
    setForm({ ...item, project_id: item.project_id||'', order_date: item.order_date?.slice(0,10)||'', file_data: '', file_name: '', file_type: '' });
    setShowModal(true);
  };
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    const data = { ...form, project_id: form.project_id||null, order_date: form.order_date||null,
      file_data: form.file_data||null, file_name: form.file_name||null, file_type: form.file_type||null };
    if (editItem) await api.put(`/admin/orders/${editItem.id}`, data);
    else await api.post('/admin/orders', data);
    setShowModal(false); fetchData();
  };

  const handleDelete = async id => {
    if (!confirm(t(lang,'confirmDelete'))) return;
    await api.delete(`/admin/orders/${id}`); fetchData();
  };

  const filtered = items.filter(i =>
    (i.issued_to||'').includes(search) || (i.subject||'').includes(search) || (i.order_number||'').includes(search)
  );
          ...
          <div key={item.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col">
            ...
            <div className="p-4 flex-1 flex flex-col gap-1">
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs font-mono text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">{item.order_number||'—'}</span>
                <span className="text-xs text-slate-400">{item.order_date?.slice(0,10)||'—'}</span>
              </div>
              <p className="font-bold text-slate-800 text-sm mt-1 line-clamp-2">{item.subject||'—'}</p>
              <p className="text-xs text-slate-500">👤 {item.issued_to||'—'}</p>
