...
const DEFAULT = { reference_number: '', letter_date: '', recipient: '', subject: '', project_id: '', file_data: '', file_name: '', file_type: '' };
...
function FilePreview({ file_data, file_name, file_type }) {
  ...
  return (
    <div className="h-40 bg-blue-50 rounded-xl flex flex-col items-center justify-center gap-2 border border-blue-100">
      <span className="text-5xl">📋</span>
      <span className="text-xs text-blue-600 font-semibold px-2 text-center truncate w-full text-center">{file_name}</span>
    </div>
  );
}
...
function viewFile(file_data) {
  window.open(file_data, '_blank');
}
...
export default function OutgoingLetters() {
  ...
  const fetchData = async () => {
    ...
    const [l, p] = await Promise.all([api.get(`/admin/outgoing${qs}`), api.get('/projects')]);
  ...
  const handleSubmit = async e => {
    ...
    if (editItem) await api.put(`/admin/outgoing/${editItem.id}`, data);
    else await api.post('/admin/outgoing', data);
    setShowModal(false); fetchData();
  };

  const handleDelete = async id => {
    if (!confirm(t(lang, 'confirmDelete'))) return;
    await api.delete(`/admin/outgoing/${id}`); fetchData();
  };

  const filtered = items.filter(i =>
    (i.recipient||'').includes(search) || (i.subject||'').includes(search) || (i.reference_number||'').includes(search)
  );
        ...
        {filtered.map(item => (
          <div key={item.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col">
            ...
            <div className="px-4 pb-4 flex gap-2 flex-wrap">
              {item.file_data && <>
                <button onClick={() => viewFile(item.file_data)} className="flex-1 py-1.5 text-xs bg-blue-50 text-blue-600 rounded-lg font-semibold hover:bg-blue-100 transition-colors">👁 {lang==='ar'?'عرض':'View'}</button>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editItem ? t(lang,'edit') : t(lang,'add')}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <Field label={t(lang,'recipient')} required><input value={form.recipient} onChange={e=>setForm(f=>({...f,recipient:e.target.value}))} className="input" required /></Field>
          ...
          <Field label={lang==='ar'?'الملف (PDF أو صورة)':'File (PDF or Image)'}>
            ...
            {editItem?.file_name && !form.file_data && <p className="text-xs text-slate-400 mt-1">الملف الحالي: {editItem.file_name} (اتركه فارغاً للإبقاء عليه)</p>}
