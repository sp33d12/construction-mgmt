...
const DEFAULT = { reference_number: '', letter_date: '', recipient: '', subject: '', project_id: '' };
...
export default function OutgoingLetters() {
  ...
  const handleSubmit = async e => { e.preventDefault(); const data={...form,project_id:form.project_id||null,letter_date:form.letter_date||null}; if(editItem) await api.put(`/admin/outgoing/${editItem.id}`,data); else await api.post('/admin/outgoing',data); setShowModal(false); fetch(); };
  const handleDelete = async id => { if(!confirm(t(lang,'confirmDelete'))) return; await api.delete(`/admin/outgoing/${id}`); fetch(); };

  const filtered = items.filter(i => (i.recipient||'').includes(search) || (i.subject||'').includes(search) || (i.reference_number||'').includes(search));
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>{[t(lang,'refNumber'),t(lang,'letterDate'),t(lang,'recipient'),t(lang,'subject'),t(lang,'project'),t(lang,'actions')].map(h=><th key={h} className="px-4 py-3 text-start text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">{h}</th>)}</tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {filtered.map(item=>(
                <tr key={item.id}>
                  ...
                  <td className="px-4 py-3 font-medium text-slate-800">{item.recipient||'—'}</td>
